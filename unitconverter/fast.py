...
1 lb.
cremini mushrooms, halved or quartered if large 3 tbsp.
olive oil, dividedKosher salt and freshly ground black pepper
...
def masterConvert(recipe, unitFrom, unitTo, fuzzyMath=True):
	...
	for lineindex, line in enumerate(recipe):
		for wordindex, word in enumerate(line):
			if type(word) == float:
				recipe[lineindex][wordindex] = round(float(word), (1 if fuzzyMath else 3))
			for unitType in units[system]:
